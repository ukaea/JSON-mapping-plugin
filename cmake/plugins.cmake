...
macro( uda_plugin )
  ...
  set( optionArgs )
  set( oneValueArgs NAME LIBNAME ENTRY_FUNC DESCRIPTION EXAMPLE VERSION COPY_ASSET_DIR COPY_ASSET_FILE)
  set( multiValueArgs SOURCES EXTRA_INCLUDE_DIRS EXTRA_LINK_DIRS CONFIG_FILE EXTRA_LINK_LIBS EXTRA_DEFINITIONS EXTRA_INSTALL_FILES )
  ...
  if( NOT "${PLUGIN_CONFIG_FILE}" STREQUAL "" )
    foreach( PLUGIN_CONFIG_FILE_EACH ${PLUGIN_CONFIG_FILE} )
      configure_file(
        "${CMAKE_CURRENT_LIST_DIR}/${PLUGIN_CONFIG_FILE_EACH}.in"
        "${CMAKE_BINARY_DIR}/etc/plugins.d/${PLUGIN_CONFIG_FILE_EACH}"
        @ONLY
      )
      install(
        FILES "${CMAKE_BINARY_DIR}/etc/plugins.d/${PLUGIN_CONFIG_FILE_EACH}"
  ...
  if( NOT "${PLUGIN_COPY_ASSET_DIR}" STREQUAL "" )
    ...
    install(
      DIRECTORY "${CMAKE_CURRENT_LIST_DIR}/${PLUGIN_COPY_ASSET_DIR}/"
      DESTINATION "etc/plugins.d/${PLUGIN_ASSET_DIRECTORY}"
  )
  endif()
    if( NOT "${PLUGIN_COPY_ASSET_FILE}" STREQUAL "" )
    file(COPY "${CMAKE_CURRENT_LIST_DIR}/${PLUGIN_COPY_ASSET_FILE}/" DESTINATION "${CMAKE_BINARY_DIR}/etc/plugins.d/${PLUGIN_COPY_ASSET_FILE}")
    string(TOLOWER "${PLUGIN_NAME}" PLUGIN_ASSET_DIRECTORY)
    install(
      FILES "${CMAKE_BINARY_DIR}/etc/plugins.d/${PLUGIN_COPY_ASSET_FILE}"
      DESTINATION "etc/plugins.d/${PLUGIN_ASSET_DIRECTORY}"
  )
  endif()
