...
namespace DRaFT_plugin_helpers {
     ...
     * @return
     */
    int get_data(DATA_BLOCK* data_block, std::string_view key, std::string_view var, int shot){
        ...
        if (key == "/AMC/PLASMA_CURRENT") { // temporary plasma_current test
            ...
            imas_json_plugin::uda_helpers::setReturnDataArrayType_Vec<float>(data_block, vec_values);
        }
        return 0;
    }
}
