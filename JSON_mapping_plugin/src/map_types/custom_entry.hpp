...
enum class CustomMapType_t {
    MASTU_helloworld,
    DRAFT_helloworld,
    INVALID
};
NLOHMANN_JSON_SERIALIZE_ENUM(CustomMapType_t,
        {{CustomMapType_t::INVALID, nullptr},
         {CustomMapType_t::MASTU_helloworld, "MASTU_helloworld"},
         {CustomMapType_t::DRAFT_helloworld, "DRAFT_helloworld"}});

/**
 * @class CustomEntry
 * @brief Mapping class CustomEntry to hold the CUSTOM MAP_TYPE from the JSON mapping files. Custom functions are able to be executed using the CustomMapType_t variable, any unrecognised string reverts to INVALID type and returns 1
 *
 */
class CustomEntry : public Mapping {
  public:
    ...
    explicit CustomEntry(CustomMapType_t custom_type) : m_custom_type(custom_type) {};
